#
# shell.py - the winner main shell
#
# This file if part of winner project
#
...
def print_startup():
    ...
    print("\033[00m")
    sys.stdout.write("\t  .:[ Winner last version (%s-%s) ]:.\n\033[00m" %(__version__,  __codename__,))
...
def shell():
    ...
    while True:
        try:
            # for Re-complete
            complete(array)
            an = raw_input("root@XnuversXploitXen >> ") or "help"
            prompt = an.split()
            if not prompt:
              continue
                ...
                if prompt[1] == "port":
                    ...
                    if port == 80 and os.getuid() != 0:
                        printt(2, "Permission denied, to bind port 80, you need to run winner as root.");
